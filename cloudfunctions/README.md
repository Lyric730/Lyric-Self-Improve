...
- `match`：处理比赛房间、比赛查询和服务端结算；`createRoom` 创建等待房间；`get` 读取房间状态；`previewSettlement` 只计算结算预览、不写库；`settle` 会计算积分、随机奖励、段位变化，写入 `settlements`、`points_ledger`、`member_points` 和 `matches.status`；`getSettlement` 读取已结算记录。
