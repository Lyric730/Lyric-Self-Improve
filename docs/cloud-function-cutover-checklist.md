...
| `admin` | `getMemberForRole` | 老板端人员权限 | 扫会员码后读取该会员当前身份、昵称和积分 |
| `admin` | `setMemberRole` | 老板端人员权限 | 只能由 owner 把会员设为 `staff` / `screen` / `player`，不能修改 owner |
...
2. 门店初始化页读取当前 OpenID；在未初始化时输入一次性密钥，绑定首个老板账号。
...
9. 老板端扫码会员码，把一个测试微信设为员工；该微信重新进入后能打开前台工作台。
10. 老板端把该测试微信改回普通球友；该微信重新进入后不能打开前台工作台。
11. 玩法选择和底分倍率页显示老板端配置的玩法、底分、倍率和随机奖励。
12. 大屏页显示老板端配置的主榜、副榜、刷新文案。
13. 球友端完整走一场挑战，结算由云函数完成，重复点击不能重复结算。
