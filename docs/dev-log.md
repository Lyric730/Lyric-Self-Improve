...
- 用户反馈：上一版图片没有抠干净，部分素材截图不完整。
- 根因：
  - 从整张资产板手动估坐标裁切，容易带入相邻素材和分割线。
  - 只看黑底预览不够，黑色背景会掩盖透明边缘脏线。
- 调整：
  - 收紧宝箱、结算徽章、段位卡等裁切框，去掉右侧竖线残留。
  - 抠图脚本每次生成前先清空 `miniprogram/assets/ui-kit/*.png`，避免旧资产残留。
  - 之后检查抠图必须同时看黑底预览和棋盘格透明预览。
- 当前正式保留 15 个资产；星级小星星边缘暂不合格，先不纳入正式资产库。

### UI Kit 第七轮：左侧截断修正

- 用户反馈：素材左边没有截完整。
- 新增硬校验：逐张 PNG 检查四条边是否存在非透明像素；只要边界有像素，就视为有截断风险。
- 脚本修正：
  - 源裁切框保持右边界基本不变，只向左补回内容，避免右侧相邻素材被带入。
  - 输出 PNG 统一增加透明安全边距，避免素材贴边。
- 验证结果：
  - `Edge check OK`：15 张 PNG 四条边均无非透明像素。
  - 新增棋盘格透明预览：`docs/design/extracted-ui-assets-checker-preview.png`。
